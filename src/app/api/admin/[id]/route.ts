...
export async function PUT(request:any, content:any) {
   ...
   await Connect();
   const data = await adminUserModelData.findOneAndUpdate(filter,payload)
...
export async function GET(request:any, content:any) {
   ...
   const record = {_id:id}
   await Connect();
   const data = await adminUserModelData.findById(record)
    return NextResponse.json({data, success:true})}
...
export async function DELETE(request:any, content:any) {
   ...
   const record = {_id:id}
   await Connect();
   const data = await adminUserModelData.deleteOne(record)
