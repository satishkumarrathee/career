...
export async function POST(req: Request) {
  try {
    ...
    const { name, contact, email, address1, address2, state, district, pincode, imgUrl } = body;
    const University = new universityModelData({ name, contact, email, address1, address2, state, district, pincode, imgUrl });
...
export async function GET() {
  ...
  try {
    await Connect();
    data = await universityModelData.find();
  } catch (err) {
    data = { err }
    success = false
  }
