...
export async function PUT(request:any, content:any) {
   ...
   await Connect();
   const data = await careerModelData.findOneAndUpdate(filter,payload)
...
export async function GET(request:any, content:any) {
   ...
   const record = {_id:id}
   await Connect();
   const data = await careerModelData.findById(record)
    return NextResponse.json({data, success:true})}
...
export async function DELETE(request:any, content:any) {
   ...
   const record = {_id:id}
   await Connect();
   const data = await careerModelData.deleteOne(record)
