...
const ManageBanner = () => {
    ...
    const uploadFile = (file: File, fileType: string) => {
        ...
        const folder = fileType === "imgUrl" ? "bannerImages/" : "bannervideos/";
        ...
        <Container>
            <Row className='d-flex align-items-center justify-content-center flex-column'>
                <Col md={4}className='d-flex align-items-center justify-content-center flex-column' >
                    <form onSubmit={handleSubmit} className='m-3 border shadow bg-light p-4 rounded' >
                    <div className='my-4'>
                            <TextField fullWidth type="text" id="title" value={title} onChange={(e: any) => setTitle(e.target.value)} label="Title" variant="outlined" />
                        </div>
                        <div>
                            <label htmlFor="img">Banner 2000*700 : </label> {imgPerc > 0 && "Uploading: " + imgPerc + "%"}
