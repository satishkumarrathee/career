...
const UpdateCounsellor = () => {
   ...
   useEffect(() => {
       const pullData = async () => {
           let singleData = await axios.get(`/api/counsellor/${params.id}`).then((res) => (res.data.data)).
   ...
   const handleUpdate = async () => {
       ...
       const pushData = await fetch(`/api/counsellor/${params.id}`, {
           method: 'Put',
           body: JSON.stringify({ name,experience, message,...inputs }),
           headers: { "Content-Type": "application/json" }
       })
       await pushData.json();
       toast.success("Counsellor Update")
    ...
    return (
        <Container>
            ...
            <Col md={12}>
                    <div>
                        <AdminHeading title='Update Counsellor' center />
                    </div>
                </Col>
                <hr />
                <Col md={6}>
