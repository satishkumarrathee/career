...
const ExamForm: React.FC<ExamFormProps> = ({ exam }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        let response = await fetch("/api/exam", {
            method: "POST",
            body: JSON.stringify({ gender, name, contact, exam, email, state, district, dateOfBirth }),
            mode: "cors",
            cache: "no-cache",
                    ...
                    <Col md={12}>
                        <form className='bg-light rounded shadow m-1 p-4' onSubmit={handleSubmit}>
                            <div className="text-center">
                                ...
                                <p className="fs-4 text-secondary">Discovering the Best {exam}arun Preparation Institutes in India</p>
                            </div>
                      ...
                      (item) => (
                        item.districts.map((data) => (
                          <MenuItem value={data} key={data}>
                            ...
                            <div className="d-flex justify-content-center align-items-center">
                                <Button type='submit' variant="contained" color="primary" className='mt-4'>Find Latest Exam Institute</Button></div>
                        </form>
                    </Col>
                </Row>
            </Container>
